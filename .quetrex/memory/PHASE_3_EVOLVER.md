...
python3 .quetrex/scripts/auto-refactor.py --pattern=pattern-typescript-strict

# Interactive mode (approve each change)
python3 .quetrex/scripts/auto-refactor.py --interactive

# Full auto (low + medium risk)
python3 .quetrex/scripts/auto-refactor.py --risk=medium --auto-commit
```
...
**Storage:** `.quetrex/metrics/history.json` (local JSON database)

**File:** `.quetrex/scripts/dashboard-generator.py`

Generates HTML dashboard from metrics data.

**Output:** `.quetrex/metrics/dashboard.html`
...
**File:** `.quetrex/scripts/pattern-evolution.py`
...
```bash
# Propose new pattern
python3 .quetrex/scripts/pattern-evolution.py propose \
  --name="Custom Hook: useAsyncOperation" \
  --category="State Management" \
  --from-files="src/hooks/useAsync*.ts"

# Move pattern from TRIAL to ADOPTED
python3 .quetrex/scripts/pattern-evolution.py promote \
  --pattern=pattern-use-async-operation

# Deprecate pattern
python3 .quetrex/scripts/pattern-evolution.py deprecate \
...
python3 .quetrex/scripts/pattern-evolution.py health \
...
**File:** `.quetrex/scripts/pattern-sync.py`
...
**Log location:** `.quetrex/metrics/audit.log`
...
Access at: `.quetrex/metrics/dashboard.html`
...
```cron
# .quetrex/config/crontab

# Daily auto-refactoring (2 AM)
0 2 * * * cd /path/to/quetrex && python3 .quetrex/scripts/auto-refactor.py --risk=low --auto-commit

# Weekly pattern learning (Monday 8 AM)
0 8 * * 1 cd /path/to/quetrex && python3 .quetrex/scripts/pattern-learner.py --propose

# Weekly metrics collection (Monday 8:30 AM)
30 8 * * 1 cd /path/to/quetrex && python3 .quetrex/scripts/metrics-collector.py && python3 .quetrex/scripts/dashboard-generator.py

# Monthly pattern evolution (1st Monday 9 AM)
0 9 1-7 * * [ $(date +\%u) -eq 1 ] && cd /path/to/quetrex && python3 .quetrex/scripts/pattern-evolution.py health --all

# Quarterly cross-project sync (1st Monday of Q1,Q2,Q3,Q4)
0 10 1-7 1,4,7,10 * [ $(date +\%u) -eq 1 ] && cd /path/to/quetrex && python3 .quetrex/scripts/pattern-sync.py sync
```
