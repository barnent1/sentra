# Quetrex AI Agent Security Architecture
...
This document defines Quetrex's security architecture for running AI agents that execute untrusted code. Based on comprehensive research into Claude Code for Web's production architecture, Quetrex implements a pragmatic 3-phase approach that balances security, engineering effort, and time-to-market.
...
Quetrex's AI agents run code generated by Claude (Anthropic's LLM) on GitHub Actions infrastructure. This creates several attack vectors:
...
Location: `/Users/barnent1/Projects/quetrex/.claude/docker/Dockerfile`
...
```bash
# Should fail
docker run --rm quetrex-ai-agent:latest touch /etc/test.txt
...
**Verification:**
```bash
docker run --rm quetrex-ai-agent:latest whoami
...
**Verification:**
```bash
docker run --rm quetrex-ai-agent:latest capsh --print
...
docker run --rm quetrex-ai-agent:latest env | grep -E "(GITHUB_TOKEN|ANTHROPIC_API_KEY)"
# Expected: No output

# Test 2: Proxy socket accessible from container
docker run --rm -v /var/run/credential-proxy.sock:/var/run/credential-proxy.sock \
  quetrex-ai-agent:latest ls -la /var/run/credential-proxy.sock
...
| Compatibility | 100% | 95% | Good enough for Quetrex |
...
Quetrex currently uses CLI subprocess invocation:
...
docker run --rm quetrex-ai-agent:latest bash -c 'echo test > /tmp/test.txt && cat /tmp/test.txt'
# Expected: "test"

# Test 3: /tmp is not executable
docker run --rm quetrex-ai-agent:latest bash -c 'echo "#!/bin/sh" > /tmp/test.sh && chmod +x /tmp/test.sh && /tmp/test.sh'
# Expected: "Permission denied"
```
...
docker run --rm --pids-limit=10 quetrex-ai-agent:latest bash -c 'for i in {1..100}; do ( sleep infinity ) & done'
# Expected: "Resource temporarily unavailable"

# Test 5: Memory limit enforced
docker run --rm --memory=100m quetrex-ai-agent:latest python3 -c "a = [0] * 10**9"
# Expected: "Killed" (OOM)
```
...
docker run --rm quetrex-ai-agent:latest python3 -c "import socket; s = socket.socket(); s.bind(('', 80))"
# Expected: "Permission denied"
...
docker run --rm -v /var/run/credential-proxy.sock:/var/run/credential-proxy.sock quetrex-ai-agent:latest test -S /var/run/credential-proxy.sock
...
*This document is the authoritative source for Quetrex's AI agent security architecture. All implementation must follow this design unless explicitly approved by Glen Barnhardt.*
