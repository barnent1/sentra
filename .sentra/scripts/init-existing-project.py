...
"""
Sentra AI-Powered SaaS Factory - Existing Project Initializer

This script initializes Sentra for an EXISTING codebase. It analyzes the
...
def create_directory_structure() -> Dict[str, Path]:
    ...
    directories = {
        ...
        'protection': Path('.sentra') / 'protection',
    }
...
def analyze_codebase_structure() -> Dict:
    ...
    key_dirs = ['src', 'tests', 'docs', 'backend', 'prisma', '.sentra']
...
**Tool:** Sentra Existing Project Initializer

---

## Overview

This is an **existing codebase** being integrated with Sentra AI-Powered SaaS Factory.
...
Protection rules are defined in `.sentra/protection/protection-rules.yml`:
...
def print_summary(analysis: Dict, patterns: Dict) -> None:
    ...
    print("   cat .sentra/protection/protection-rules.yml")
    print()
...
def main():
    ...
    print(f"{Colors.PURPLE}║{' ' * 58}║{Colors.NC}")
    print(f"{Colors.PURPLE}║  Sentra Project Initializer - Existing Codebase{' ' * 9}║{Colors.NC}")
    print(f"{Colors.PURPLE}║{' ' * 58}║{Colors.NC}")
    print(f"{Colors.PURPLE}╚{'═' * 58}╝{Colors.NC}")
    print()
