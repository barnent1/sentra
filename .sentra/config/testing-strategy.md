...
python3 .sentra/scripts/auto-refactor.py --dry-run --pattern=typescript_any
...
python3 .sentra/scripts/auto-refactor.py --risk=high --interactive
...
# Verify: Check .sentra/metrics/history.json contains:
cat .sentra/metrics/history.json | jq '.[0] | keys'
...
python3 .sentra/scripts/metrics-collector.py --compare=1d
...
[ -f .sentra/metrics/dashboard.html ] && echo "✅ Dashboard created"
...
rm .sentra/metrics/history.json
...
# 3. Error logged to .sentra/metrics/audit.log
```
...
# Test: Auto-refactor should rollback
python3 .sentra/scripts/auto-refactor.py
...
python3 .sentra/scripts/auto-refactor.py --max-files=10
...
time python3 .sentra/scripts/metrics-collector.py
...
time python3 .sentra/scripts/dashboard-generator.py
...
Create `.sentra/tests/phase3-tests.sh`:
...
python3 .sentra/scripts/metrics-collector.py
[ -f .sentra/metrics/history.json ] && echo "✅ Metrics collected"

# Test 4: Dashboard Generator
echo "Test 4: Dashboard Generator..."
python3 .sentra/scripts/dashboard-generator.py
[ -f .sentra/metrics/dashboard.html ] && echo "✅ Dashboard generated"
...
Run with:
```bash
chmod +x .sentra/tests/phase3-tests.sh
.sentra/tests/phase3-tests.sh
```
  ...
  test-phase3:
    ...
    steps:
      ...
      - name: Test auto-refactor (dry-run)
        run: python3 .sentra/scripts/auto-refactor.py --dry-run --risk=low

      - name: Collect metrics
        run: python3 .sentra/scripts/metrics-collector.py

      - name: Generate dashboard
        run: python3 .sentra/scripts/dashboard-generator.py

      - name: Upload dashboard artifact
        uses: actions/upload-artifact@v3
        with:
          name: dashboard
          path: .sentra/metrics/dashboard.html
      ...
      - name: Check health score
        run: |
          ...
          python3 -c "
          import json
          with open('.sentra/metrics/history.json', 'r') as f:
              history = json.load(f)
              latest = history[-1]
