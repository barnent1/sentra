 ...
 */
export const findSimilarDnaPatterns = async (queryEmbedding, options = {}) => {
 ...
 */
export const findSimilarLearningOutcomes = async (queryEmbedding, options = {}) => {
    const db = getDatabase();
    const { limit = 10, threshold = 0.7, operator = 'cosine' } = options;
    const operatorMap = {
        cosine: '<=>',
 ...
 */
export const updateDnaEmbedding = async (dnaId, embedding) => {
    const db = getDatabase();
    await db
        .update(evolutionDna)
        .set({
 ...
 */
export const updateLearningOutcomeEmbedding = async (outcomeId, embedding) => {
    const db = getDatabase();
    await db
 ...
 */
export const generateRandomEmbedding = (dimensions = 1536) => {
    const vector = [];
