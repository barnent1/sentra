...
export const projects = pgTable('projects', {
  id: uuid('id').primaryKey().$type<ProjectContextId>().default(uuidGenerateV4),
...
export const tasks = pgTable('tasks', {
  id: uuid('id').primaryKey().$type<TaskId>().default(uuidGenerateV4),
  projectId: uuid('project_id').$type<ProjectContextId>().references(() => projects.id),
  ...
  parentTaskId: uuid('parent_task_id').$type<TaskId>(),
...
export const events = pgTable('events', {
  ...
  taskId: uuid('task_id').$type<TaskId>().references(() => tasks.id),
