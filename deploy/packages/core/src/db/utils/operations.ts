...
export const recordLearningOutcome = async (outcomeData: {
  ...
  const [outcome] = await db
    ...
    .values({
      ...
      taskId: outcomeData.taskId as any, // Cast to UUID type
 ...
 */
export const getAgentInstanceWithContext = async (agentId: AgentInstanceId) => {
  const db = getDatabase();
 ...
 */
export const findBestDnaForProject = async (
 ...
 */
export const getEvolutionLineage = async (dnaId: EvolutionDnaId) => {
  const db = getDatabase();
  ...
  return {
    ancestors: ancestors as any[], // Type casting for simplicity
