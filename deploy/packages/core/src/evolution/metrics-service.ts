...
export interface MetricsAggregation {
  ...
  readonly percentile?: number; // for percentile aggregations
}

export interface EvolutionTrend {
  readonly metricType: EvolutionMetricType;
  readonly direction: 'increasing' | 'decreasing' | 'stable';
  readonly rate: number; // rate of change per hour
  readonly confidence: number; // 0-1, confidence in trend direction
...
export interface MetricsServiceConfig {
  readonly retention: {
    readonly raw: number; // milliseconds to keep raw metrics
    readonly hourly: number; // milliseconds to keep hourly aggregations
    readonly daily: number; // milliseconds to keep daily aggregations
  };
  readonly aggregation: {
    readonly enabled: boolean;
    readonly intervals: readonly number[]; // aggregation intervals in milliseconds
    readonly batchSize: number;
  };
  readonly alerts: {
    readonly enabled: boolean;
    readonly thresholds: Record<string, { min?: number; max?: number; rate?: number }>;
    readonly suppressionTime: number; // milliseconds to suppress repeat alerts
  };
  readonly realtime: {
    readonly enabled: boolean;
...
export class EvolutionMetricsService extends EventEmitter {
  ...
  private readonly rawMetrics: EvolutionMetricPoint[] = [];
  private readonly aggregatedMetrics = new Map<string, MetricsAggregation[]>();
  ...
  recordMetric(
    ...
    const metricPoint: EvolutionMetricPoint = {
      id: `metric_${Date.now()}_${Math.random().toString(36).slice(2)}` as MetricId,
      timestamp: new Date(),
      metricType,
      value,
  ...
  getMetrics(
    ...
    return this.rawMetrics.filter(metric => {
      ...
      if (filters?.projectId && metric.projectId !== filters.projectId) return false;
      if (filters?.dnaId && metric.dnaId !== filters.dnaId) return false;
      return true;
    });
  }
   ...
   */
  calculateTrends(
    metricTypes: readonly EvolutionMetricType[],
    timeSpan: number = 60 * 60 * 1000 // 1 hour
  ...
  generateDashboard(): RealTimeEvolutionDashboard {
    ...
    const topPerformers: RealTimeEvolutionDashboard['topPerformers'] = this.getTopPerformers();

    // Get recent evolutions
    const recentEvolutions: RealTimeEvolutionDashboard['recentEvolutions'] = this.getRecentEvolutions();
  ...
  getSystemHealthScore(): number {
    ...
    const healthScore = (
      avgFitness * weights.averageFitness +
  ...
  private checkAlerts(metric: EvolutionMetricPoint): void {
  ...
  private determineSeverity(
    ...
    if (criticalMetrics.includes(metricType as any) && Math.abs(deviation) > 0.5) {
      return 'critical';
  ...
  private calculateTrend(
    metrics: readonly EvolutionMetricPoint[],
    timeSpan: number
  ): EvolutionTrend | null {
    ...
    let direction: EvolutionTrend['direction'];
    ...
    let significance: EvolutionTrend['significance'];
  ...
  private getLatestMetric(metricType: EvolutionMetricType): EvolutionMetricPoint | undefined {
  ...
  private getTopPerformers(): RealTimeEvolutionDashboard['topPerformers'] {
    ...
    return recentFitnessMetrics.map(metric => ({
      ...
      dnaId: metric.dnaId!,
      fitness: metric.value as FitnessScore,
      generation: (metric.metadata['generation'] as number) ?? 1,
      projectContext: (metric.metadata['projectType'] as string) ?? 'Unknown',
    }));
  }

  private getRecentEvolutions(): RealTimeEvolutionDashboard['recentEvolutions'] {
    // Mock implementation - would query from database in production
    ...
    return recentEvolutions.map(metric => ({
      agentId: metric.agentId!,
      dnaId: metric.dnaId!,
      parentDnaId: (metric.metadata['parentDnaId'] as EvolutionDnaId) ?? undefined,
      fitnessImprovement: metric.value,
      timestamp: metric.timestamp,
      evolutionType: (metric.metadata['evolutionType'] as string) ?? 'mutation',
    }));
  }

  private getActiveAlerts(): readonly EvolutionAlert[] {
  ...
  private getProjectBreakdown(): RealTimeEvolutionDashboard['projectBreakdown'] {
    // Mock implementation - would query from database in production
    const projectMetrics = new Map<ProjectContextId, {
      agents: Set<AgentInstanceId>;
      fitnessSum: number;
      fitnessCount: number;
      activity: number;
    }>();
    ...
    this.rawMetrics
      ...
      .forEach(metric => {
        ...
        if (metric.agentId) existing.agents.add(metric.agentId);
