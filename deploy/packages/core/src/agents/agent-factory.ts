...
// UUID import removed as not used
import type { 
  AgentInstanceId,
  AgentType,
  CodeDNA,
...
export class AgentFactory {
  private readonly agentRegistry = new Map<AgentType, typeof BaseEvolutionaryAgent>();
  ...
  async spawnAgent(config: AgentCreationConfig): Promise<AgentSpawnResult> {
    try {
      const agentId = `agent_${config.type}_${Date.now()}_${this.spawnCount++}` as AgentInstanceId;
      ...
      const agentId = `failed_${config.type}_${Date.now()}` as AgentInstanceId;
      const dnaId = `failed_dna_${Date.now()}` as EvolutionDnaId;
   ...
   */
  private generateAgentDNA(config: AgentCreationConfig, _agentId: AgentInstanceId): CodeDNA {
    ...
    const genetics: EnhancedGeneticMarkers = {
      ...baseGenetics,
      ...config.customGenetics,
    };
    
    const performance: EnhancedPerformanceMetrics = {
      ...basePerformance,
      ...config.customPerformance,
    };
    
    const dnaId = `dna_${config.type}_${Date.now()}` as EvolutionDnaId;
    ...
    const baseDna = {
      ...
      genetics,
      performance,
      mutations: [] as readonly Mutation[],
      embedding: this.generateEmbedding(config.type, config.projectContext),
      timestamp: new Date(),
      generation: 0,
      birthContext: {
        trigger: 'agent_spawn' as const,
        creationReason: `Spawned ${config.type} agent for ${config.projectContext.projectType} project`,
        initialPerformance: performance,
      },
      evolutionHistory: [] as readonly string[],
      activationCount: 0,
      lastActivation: new Date(),
      fitnessScore: this.calculateInitialFitness(genetics, performance) as FitnessScore,
      viabilityAssessment: {
      ...
      tags: [config.type, config.projectContext.projectType, config.projectContext.complexity] as readonly string[],
   ...
   */
  private getOptimalGeneticsForType(type: AgentType): EnhancedGeneticMarkers {
    const baseGenetics: EnhancedGeneticMarkers = {
   ...
   */
  private getExpectedPerformanceForType(type: AgentType): EnhancedPerformanceMetrics {
    const basePerformance: EnhancedPerformanceMetrics = {
      successRate: 0.85,
   ...
   */
  private getAgentClass(type: AgentType): typeof BaseEvolutionaryAgent {
   ...
   */
  private generateEmbedding(type: AgentType, context: ProjectContext): readonly number[] {
   ...
   */
  private calculateInitialFitness(genetics: EnhancedGeneticMarkers, performance: EnhancedPerformanceMetrics): number {
    const geneticsScore = (Object.values(genetics) as number[]).reduce((sum: number, val: number) => sum + val, 0) / Object.keys(genetics).length;
   ...
   */
  private getPatternTypeForAgent(type: AgentType): string {
    const patterns = {
    ...
    return patterns[type as keyof typeof patterns] || 'general';
  }

  /**
   * Get strengths for agent type
   */
  private getStrengthsForType(type: AgentType): readonly string[] {
    const strengths = {
    ...
    return strengths[type as keyof typeof strengths] || ['General problem solving'];
  }

  /**
   * Get weaknesses for agent type
   */
  private getWeaknessesForType(type: AgentType): readonly string[] {
    const weaknesses = {
    ...
    return weaknesses[type as keyof typeof weaknesses] || ['Specialized domains'];
  }

  /**
   * Get recommended contexts for agent type
   */
  private getRecommendedContexts(type: AgentType): readonly string[] {
    const contexts = {
    ...
    return contexts[type as keyof typeof contexts] || ['general'];
  }

  /**
   * Get contexts to avoid for agent type
   */
  private getAvoidContexts(type: AgentType): readonly string[] {
    const avoidContexts = {
    ...
    return avoidContexts[type as keyof typeof avoidContexts] || [];
  }
