...
export interface AgentDecisionEvent extends BaseEvent {
  ...
  readonly data: {
    ...
    readonly confidence: number;              // 0-1
    readonly reasoning: string;
    readonly factors: Record<string, number>; // Decision factors and weights
  };
}
...
export interface PerformancePulseEvent extends BaseEvent {
  ...
  readonly data: {
    ...
    readonly healthScore: number;           // 0-1, overall agent health
    readonly activities: readonly {
      readonly activity: string;
      readonly progress: number;            // 0-1
      readonly estimatedCompletion: number; // milliseconds
    }[];
  };
}
...
export const isObservabilityEvent = (event: BaseEvent): event is
  ToolUsageStartedEvent | ToolUsageCompletedEvent | AgentDecisionEvent | 
  AgentMemoryOperationEvent | AgentCoordinationEvent | PerformancePulseEvent | 
  AgentLearningPatternEvent =>
  event.category === EventCategory.AUDIT || 
