...
export interface SessionScalingConfig {
  ...
  readonly projectsPerSession: 4; // Always 4 for the grid
 ...
 */
export const isValidSessionId = (value: unknown): value is SessionId => {
  return typeof value === 'string' && value.length > 0;
};

export const isValidPanelPosition = (value: unknown): value is PanelPositionType => {
  return Object.values(PanelPosition).includes(value as PanelPositionType);
};
...
export const isWebSocketMessage = (value: unknown): value is WebSocketMessage => {
  return (
    ...
    'timestamp' in value &&
    'sessionId' in value &&
    Object.values(WebSocketMessageType).includes((value as any).type)
  );
};
