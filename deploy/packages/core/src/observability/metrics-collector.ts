...
export interface MetricDefinition {
  ...
  readonly interval: number;              // Collection interval in ms
...
export class BuiltInCollectors {
   ...
   */
  static responseTime: MetricCollector = async (agentId, context) => {
   ...
   */
  static cpuUsage: MetricCollector = async (agentId, context) => {
   ...
   */
  static memoryUsage: MetricCollector = async (agentId, context) => {
   ...
   */
  static taskCompletionRate: MetricCollector = async (agentId, context) => {
   ...
   */
  static errorRate: MetricCollector = async (agentId, context) => {
   ...
   */
  static learningProgress: MetricCollector = async (agentId, context) => {
   ...
   */
  static collaborationScore: MetricCollector = async (agentId, context) => {
   ...
   */
  static throughput: MetricCollector = async (agentId, context) => {
   ...
   */
  static qualityScore: MetricCollector = async (agentId, context) => {
   ...
   */
  static resourceEfficiency: MetricCollector = async (agentId, context) => {
...
export class MetricsCollector {
  ...
  private readonly collectionTimers = new Map<string, NodeJS.Timeout>();
  private readonly lastCollectionTime = new Map<string, Date>();

  // Active agents to monitor
  private readonly monitoredAgents = new Set<AgentInstanceId>();

  // Collected metrics buffer
  private readonly metricsBuffer: CollectedMetric[] = [];
  ...
  addAgent(agentId: AgentInstanceId): void {
    if (this.monitoredAgents.has(agentId)) return;
   ...
   */
  private stopMetricCollection(metricName: string): void {
   ...
   */
  private async collectMetricForAllAgents(metricName: string): Promise<void> {
    const definition = this.metricDefinitions.get(metricName);
    if (!definition || !definition.enabled) return;

    const collectionPromises = Array.from(this.monitoredAgents).map(agentId =>
   ...
   */
  private async collectMetricForAgent(metricName: string, agentId: AgentInstanceId): Promise<void> {
    const definition = this.metricDefinitions.get(metricName);
    if (!definition || !definition.enabled) return;
   ...
   */
  private buildCollectionContext(agentId: AgentInstanceId): Record<string, unknown> {
    return {
      agentId,
   ...
   */
  private triggerInitialCollection(agentId: AgentInstanceId): void {
  ...
  private performAggregation(): void {
    ...
    const windowMetrics = this.metricsBuffer.filter(m =>
      m.timestamp >= windowStart && m.timestamp <= now
    );
   ...
   */
  private groupMetrics(metrics: CollectedMetric[]): Map<string, CollectedMetric[]> {
    const grouped = new Map<string, CollectedMetric[]>();
   ...
   */
  private setupBuiltInMetrics(): void {
   ...
   */
  getRecentMetrics(agentId: AgentInstanceId, limit = 100): CollectedMetric[] {
