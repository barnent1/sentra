...
import { MetricsCollector, BuiltInCollectors } from './metrics-collector';
import { createSessionId } from './event-hooks';
...
const DEMO_CONFIG = {
  ...
  decisionTypes: ['tool_selection', 'approach_choice', 'parameter_tuning', 'strategy_pivot'] as const,
};
...
export class ObservabilityDemoSimulator {
  ...
  // Demo agents
  private readonly demoAgents: Array<{
    id: AgentInstanceId;
    name: string;
   ...
   */
  private setupDemoAgents(): void {
    this.demoAgents.push(
      {
        id: 'agent-dev-001' as AgentInstanceId,
      ...
      {
        id: 'agent-orch-001' as AgentInstanceId,
      ...
      {
        id: 'agent-spec-001' as AgentInstanceId,
  ...
  private setupBenchmarks(): void {
    ...
    this.performanceTracker.createBenchmark(
      'Fast Response Time',
      'Tools should complete within 2 seconds for optimal UX',
      'tool_duration',
      1500, // target: 1.5 seconds
    ...
    this.performanceTracker.createBenchmark(
      'High Success Rate',
      'Tools should have >95% success rate',
      'tool_success_rate',
      0.95, // target: 95%
      0.90, // threshold: 90%
      'rate',
    ...
    this.performanceTracker.createBenchmark(
      ...
      'Agent decisions should have >80% confidence',
      'decision_confidence',
      0.80, // target: 80%
      0.70, // threshold: 70%
      'score',
  ...
  stopSimulation(): void {
    ...
    this.demoAgents.forEach(agent => {
      if (agent.sessionId) {
        this.observabilityManager.endSession(agent.sessionId as any);
      }
    });
   ...
   */
  private async simulateToolUsage(agent: typeof this.demoAgents[0]): Promise<void> {
   ...
   */
  private async simulateDecision(agent: typeof this.demoAgents[0]): Promise<void> {
   ...
   */
  private async simulateMemoryOperation(agent: typeof this.demoAgents[0]): Promise<void> {
    const operations: Array<'store' | 'retrieve' | 'update' | 'forget'> = ['store', 'retrieve', 'update', 'forget'];
    const memoryTypes: Array<'working' | 'episodic' | 'semantic' | 'procedural'> = ['working', 'episodic', 'semantic', 'procedural'];
   ...
   */
  private async simulateCoordination(agent: typeof this.demoAgents[0]): Promise<void> {
    const targetAgent = this.getRandomAgent(agent.id);
    if (!targetAgent) return;

    const coordinationTypes: Array<'delegation' | 'collaboration' | 'information_sharing' | 'conflict_resolution'> = [
   ...
   */
  private getRandomAgent(excludeId?: AgentInstanceId): typeof this.demoAgents[0] {
   ...
   */
  private chooseActivityType(): 'tool_usage' | 'decision' | 'memory_operation' | 'coordination' {
    const weights = {
   ...
   */
  private getRandomDecisionType(): typeof DEMO_CONFIG.decisionTypes[number] {
   ...
   */
  private generateToolParameters(toolName: string): Record<string, unknown> {
    const baseParams = {
      timeout: 30000,
      retries: 3,
      verbose: false,
   ...
   */
  private calculateExpectedDuration(toolName: string, efficiency: number): number {
   ...
   */
  private calculateActualDuration(expectedDuration: number, efficiency: number): number {
   ...
   */
  private calculateDecisionConfidence(efficiency: number): number {
   ...
   */
  private generateDecisionOptions(decisionType: string) {
    const options = [
      {
   ...
   */
  private generateAssociatedConcepts(): string[] {
    const concepts = [
  ...
  private generateTaskId(): TaskId {
    return `task_${Date.now()}_${Math.random().toString(36).substring(2)}` as TaskId;
  }
 ...
 */
export async function runObservabilityDemo(logger?: any): Promise<void> {
  ...
  const observabilityManager = new ObservabilityManager(
    undefined, // No event bus for demo
    {
      enabled: true,
  ...
  const simulator = new ObservabilityDemoSimulator(
