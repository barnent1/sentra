...
import { createEvolutionApi, type ServerConfig } from './index';
 ...
 */
const serverOverrides: Partial<ServerConfig> = {
  auth: {
  ...
  websocket: {
    cors: {
      origin: [...exampleConfig.api.corsOrigins] as string[],
      methods: ['GET', 'POST'],
    },
...
async function main(): Promise<void> {
  ...
  try {
    ...
    const api = createEvolutionApi(exampleConfig, serverOverrides);
...
export class ApiExample {
  ...
  constructor(baseUrl: string = `http://localhost:${exampleConfig.api.port}`) {
   ...
   */
  async createPattern(authToken: string): Promise<void> {
    try {
      const patternData = {
