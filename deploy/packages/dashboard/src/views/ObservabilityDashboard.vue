<template>
  <div class="h-screen flex flex-col bg-gray-50 dark:bg-gray-900">
    ...
    <header class="bg-gradient-to-r from-blue-600 to-blue-500 dark:from-blue-700 dark:to-blue-600 shadow-lg border-b-2 border-blue-800 dark:border-blue-900">
    ...
    >
      <div class="bg-white dark:bg-gray-800 rounded-xl shadow-2xl border border-gray-200 dark:border-gray-600 max-w-4xl w-full max-h-[90vh] overflow-hidden">
        ...
        <div class="p-6">
          <pre class="text-sm bg-gray-100 dark:bg-gray-800 p-4 rounded border border-gray-200 dark:border-gray-600 overflow-auto">{{ JSON.stringify(selectedEvent, null, 2) }}</pre>
        </div>
      </div>
    </div>
  </div>
...
import { demoGenerator } from '../utils/demoDataGenerator'

// WebSocket connection - connect to real Sentra API WebSocket
const wsUrl = (import.meta as any).env.VITE_EVOLUTION_WS_URL || 'ws://sentra.cx:8001/ws/evolution'
...
const startDemoMode = () => {
  ...
  console.log('Starting demo mode with simulated evolution events')
  
  // Generate initial batch of events
  const initialEvents = demoGenerator.generateBatch(25)
  
  // Simulate loading initial events
  initialEvents.forEach(event => {
    (events as any).value.unshift(event)
  })
  
  // Start generating live events every 2-5 seconds
  demoInterval = setInterval(() => {
    if (!(isConnected as any)?.value) {
      const newEvent = demoGenerator.generateStreamEvent()
      ;(events as any).value.unshift(newEvent)
      
      // Limit events to prevent memory issues
      if ((events as any).value.length > 100) {
        (events as any).value = (events as any).value.slice(0, 100)
      }
    }
  }, Math.random() * 3000 + 2000) as unknown as number // 2-5 second interval
}
...
const stopDemoMode = () => {
  if (demoInterval) {
    ...
    demoInterval = null
    console.log('Stopped demo mode')
  }
}

// Auto-start demo mode if not connected after 3 seconds
setTimeout(() => {
  if (!(isConnected as any)?.value && (import.meta as any).env.VITE_DEBUG_MODE === 'true') {
    startDemoMode()
  }
}, 3000)
</script>
