...
class MessageQueueClass {
  ...
  async subscribeToAgentEvents(handler: MessageHandler): Promise<void> {
  ...
  async publishTaskEvent(eventType: string, taskId: string, data: any): Promise<void> {
  ...
  async subscribeToTaskEvents(handler: MessageHandler): Promise<void> {
    await this.subscribe(
  ...
  async publishContainerEvent(eventType: string, containerId: string, data: any): Promise<void> {
    const routingKey = `container.${eventType}`;
    const message = {
      eventType,
      containerId,
      data,
  ...
  async subscribeToContainerEvents(handler: MessageHandler): Promise<void> {
    await this.subscribe(
      'orchestrator.container-events',
      ['container.*'],
      handler
    );
  }
  ...
  async requestContextInjection(agentId: string, contextIds: string[], options: any = {}): Promise<void> {
    const routingKey = 'context.request.inject';
    const message = {
      requestType: 'inject',
      contextId: '',
      data: {
  ...
  async subscribeToContextResponses(handler: MessageHandler): Promise<void> {
    await this.subscribe(
      'orchestrator.context-responses',
      ['context.response.*', 'context.injection_response'],
      handler
    );
  }

  // Quality gates and validation messages
  async publishQualityCheck(checkType: string, data: any): Promise<void> {
    const routingKey = `quality.${checkType}`;
    const message = {
      checkType,
      data,
      timestamp: new Date().toISOString(),
  ...
  async subscribeToQualityResults(handler: MessageHandler): Promise<void> {
    await this.subscribe(
      'orchestrator.quality-results',
      ['quality.result.*'],
      handler
    );
  }
  ...
  async sendToAgent(agentId: string, messageType: string, data: any): Promise<void> {
