...
router.get('/health', asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    try {
      const { DockerManager } = await import('../services/dockerManager');
      // We can't easily access the docker manager instance here,
      // so we'll assume it's healthy if the service is running
      health.checks.docker = true;
    } catch {
      health.checks.docker = false;
    }
