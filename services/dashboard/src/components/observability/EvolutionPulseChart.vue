<template>
  <div class="bg-gradient-to-r from-[var(--theme-bg-primary)] to-[var(--theme-bg-secondary)] px-3 py-4 mobile:py-3 shadow-lg">
    <div class="flex items-center justify-between mb-3 mobile:flex-col mobile:space-y-2 mobile:items-start">
      ...
      <div class="flex gap-1.5 mobile:w-full mobile:justify-center" role="tablist" aria-label="Time range selector">
        <button
          ...
          :aria-label="`Show ${range === '1m' ? '1 minute' : range === '3m' ? '3 minutes' : '5 minutes'} of evolution activity`"
    ...
    <!-- Evolution Metrics Row -->
    <div class="grid grid-cols-4 gap-2 mb-3 mobile:grid-cols-2 mobile:gap-1">
      <div class="text-center p-2 bg-[var(--theme-bg-tertiary)] rounded-lg border border-[var(--theme-border-primary)]">
        <div class="text-sm font-bold text-[var(--theme-primary)]">{{ evolutionMetrics.totalEvents }}</div>
        <div class="text-xs text-[var(--theme-text-tertiary)]">Events</div>
      </div>
      <div class="text-center p-2 bg-[var(--theme-bg-tertiary)] rounded-lg border border-[var(--theme-border-primary)]">
        <div class="text-sm font-bold text-[var(--theme-primary)]">{{ evolutionMetrics.uniqueAgentTypes }}</div>
        <div class="text-xs text-[var(--theme-text-tertiary)]">Species</div>
      </div>
      <div class="text-center p-2 bg-[var(--theme-bg-tertiary)] rounded-lg border border-[var(--theme-border-primary)]">
        <div class="text-sm font-bold text-[var(--theme-primary)]">{{ evolutionMetrics.uniqueDnaVariants }}</div>
        <div class="text-xs text-[var(--theme-text-tertiary)]">DNA Variants</div>
      </div>
      <div class="text-center p-2 bg-[var(--theme-bg-tertiary)] rounded-lg border border-[var(--theme-border-primary)]">
        <div class="text-sm font-bold text-[var(--theme-primary)]">{{ evolutionMetrics.evolutionRate.toFixed(1) }}/min</div>
...
import type { EvolutionWebSocketEvent, TimeRange } from '../../composables/useEvolutionWebSocket'
import { useEvolutionChartData } from '../../composables/useEvolutionChartData'
import { createEvolutionChartRenderer, type ChartDimensions } from '../../utils/evolutionChartRenderer'
...
const timeRanges: readonly TimeRange[] = ['1m', '3m', '5m'] as const
...
} = useEvolutionChartData()

let renderer: ReturnType<typeof createEvolutionChartRenderer> | null = null
let resizeObserver: ResizeObserver | null = null
let animationFrame: number | null = null
const processedEventIds = new Set<string>()

const hasData = computed(() => dataPoints.value.some(dp => dp.count > 0))
const evolutionMetrics = computed(() => getEvolutionMetrics())

const chartAriaLabel = computed(() => {
  const totalEvents = evolutionMetrics.value.totalEvents
  const rangeText = timeRange.value === '1m' ? '1 minute' : timeRange.value === '3m' ? '3 minutes' : '5 minutes'
  return `Evolution activity chart showing ${totalEvents} events over the last ${rangeText}`
})
...
const render = () => {
  ...
  renderer.drawEvolutionMetrics(data)
}
...
const isEventFiltered = (event: EvolutionWebSocketEvent): boolean => {
...
const processNewEvents = () => {
  const currentEvents = Array.from(props.events)
  const newEventsToProcess: EvolutionWebSocketEvent[] = []
  ...
  if (barIndex >= 0 && barIndex < data.length && y >= chartArea.y && y <= chartArea.y + chartArea.height) {
    ...
    if (point.count > 0) {
      const eventTypesText = Object.entries(point.eventTypes || {})
        .map(([type, count]) => `${formatEventTypeLabel(type)}: ${count}`)
        .join(', ')
...
onMounted(() => {
  ...
  renderer = createEvolutionChartRenderer(canvas.value, dimensions, config)
