...
- Spec File: `.quetrex/specs/screens/screen-name.yml`
...
- Patterns: `.quetrex/memory/patterns.md`
- API Spec: `.quetrex/architect-sessions/<project>/api-spec.yaml`
...
Create `.quetrex/dependency-graph.yml`:
...
2. **Generate issues in batches**
   ...
   gh issue create \
     ...
     --body-file .quetrex/issues/issue-41.md \
...
Generate `.quetrex/progress-dashboard.md`:
...
gh issue create \
  ...
  --body-file .quetrex/checkpoints/batch-3-review.md \
...
const before = parseOpenAPI('.quetrex/api-spec.yaml')
...
```yaml
# .quetrex/conflict-resolution-rules.yml
...
**Steps:**
1. Read all spec documents from `.quetrex/architect-sessions/<project>/`
...
**Output:**
- `.quetrex/breakdown-plan.md` (for human review)
...
2. Generate dependency graph (`.quetrex/dependency-graph.yml`)
...
5. Update `.quetrex/progress-dashboard.md`
...
2. Save issue content to `.quetrex/failed-issues/issue-N.md`
