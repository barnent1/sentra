# Sentra Development Workflow Skill

**Purpose:** Bootstrap new Claude Code sessions with complete Sentra project context and enable efficient issue-driven development.

**When to Use:**
- At the start of any new Claude Code session working on Sentra
...
gh workflow run "Sentra AI Agent Worker" -f issue_number=123
```
...
| `.sentra/status.yml` | Current roadmap position |
...
**Sentra** is a voice-first AI agent control center - a mission control dashboard for managing multiple AI-powered projects.
...
3. Agent Worker Executes
   ...
   └─> Loads project context from .sentra/memory/
...
`.sentra/status.yml` - Maintained snapshot of:
- Current phase
- Active focus areas
...
### Status Tracking: `.sentra/status.yml`
...
- [ ] Update `.sentra/status.yml` before ending session
