...
const comparisonData = computed(() => {
  return ((agentsStore.agents as any[]) || []).map((agent: any) => ({
    ...
    name: agent.name,
    metrics: agentsStore.getAgentMetrics(agent.id),
    currentPerformance: agent.performanceHistory?.[agent.performanceHistory.length - 1]
  })).filter(item => item.metrics && item.currentPerformance) // Only show agents with actual data
})
...
// Helper functions - these will be replaced with real data from the API
...
const getTaskProgress = (agentId: AgentInstanceId): number => {
  // Get actual task progress from the agent's current task
  const agent = agentsStore.getAgent(agentId)
  const currentTaskId = agentsStore.getAgentMetrics(agentId)?.currentTask
  if (!currentTaskId) return 0
  
  // In real implementation, this would fetch from task execution store
  // For now return 0 as placeholder until we have real task data
  return 0
}
