...
export const useEvolutionStore = defineStore('evolution', () => {
  ...
  const fetchEvolutionData = async (): Promise<void> => {
    try {
      ...
      console.log('Fetching evolution data from API:', `${apiUrl}/api/evolution/patterns`)
      
      const response = await fetch(`${apiUrl}/api/evolution/patterns`, {
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${(import.meta as any).env.VITE_API_TOKEN || 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..VG2-xdMnahBUAGuaNe0VmCQPE0ieysTmnfWQxeqmiZs'}`
        }
      })
      ...
      console.log(`Loaded ${dnaPatterns.value.size} DNA patterns and ${evolutionEvents.value.length} evolution events from API`)
