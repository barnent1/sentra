...
export class EvolutionEventBroadcaster {
  ...
  constructor(
    ...
    try {
      ...
      this.logger?.error('Failed to initialize WebSocket server', { error: error instanceof Error ? error.message : error });
      throw error;
    }
  }
  ...
  private readonly setupEventHandlers = (): void => {
    if (!this.io) {
      this.logger?.error('Cannot setup event handlers: Socket.IO server not initialized');
      return;
    }
