...
export class MockDrizzleDB {
  private patterns: Map<EvolutionDnaId, MockEvolutionPattern> = new Map();
  private results: Map<string, MockEvolutionResult> = new Map();

  // Evolution Patterns Table Mock
  evolutionPatterns = {
    insert: jest.fn().mockImplementation(async (data: unknown) => {
      const typedData = data as Partial<MockEvolutionPattern>;
      const pattern: MockEvolutionPattern = {
        id: (typedData.id || `pattern-${Date.now()}`) as EvolutionDnaId,
    ...
    update: jest.fn().mockImplementation(async (args: unknown) => {
      const { 
        ...
        where: { id: EvolutionDnaId }; 
        data: Partial<MockEvolutionPattern>; 
      };
    ...
    delete: jest.fn().mockImplementation(async (args: unknown) => {
      const { where } = args as { where: { id: EvolutionDnaId } };
  ...
  evolutionResults = {
    insert: jest.fn().mockImplementation(async (data: unknown) => {
      const typedData = data as Partial<MockEvolutionResult>;
      const result: MockEvolutionResult = {
        id: typedData.id || `result-${Date.now()}`,
        patternId: typedData.patternId || ('pattern-1' as EvolutionDnaId),
        generation: typedData.generation || 1,
  ...
  transaction = jest.fn().mockImplementation(async (callback: unknown) => {
    const typedCallback = callback as (tx: MockDrizzleDB) => Promise<any>;
...
export const createMockDatabase = (): MockDrizzleDB => {
