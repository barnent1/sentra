 ...
 */
export async function seedAgentInstances(dnaPatterns) {
    const db = getDatabase();
 ...
 */
export async function seedLearningOutcomes(agents) {
    const db = getDatabase();
    console.log('📚 Seeding learning outcomes...');
    const outcomeSeeds = [];
    const outcomeTypes = ['success', 'failure', 'partial', 'blocked'];
    const lessons = [
    ...
    for (const agent of agents.slice(0, 5)) {
        for (let i = 0; i < 3; i++) {
            ...
            if (outcomeType && lesson) {
                outcomeSeeds.push({
                    id: uuidv4(), // LearningOutcomeId
                    agentInstanceId: agent.id,
                    evolutionDnaId: agent.evolutionDnaId,
                    taskId: uuidv4(), // TaskId - will need to be real task IDs in production
                    outcomeType,
...
//# sourceMappingURL=evolution-seed.js.map