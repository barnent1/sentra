...
import { AgentActivityStatus, } from './types';
/**
...
export class SessionScalingManager extends EventEmitter {
    ...
    scalingConfig;
    loadBalancingConfig;
    scalingMetrics = new Map();
    scalingInterval;
    metricsCollectionInterval;
    lastScalingAction = new Date();
    cooldownPeriod;
    constructor(sessionManager, scalingConfig, loadBalancingConfig) {
        super();
    ...
    setupEventHandlers() {
        this.on('scaling-decision', (decision) => {
     ...
     */
    makeScalingDecision(metrics, sessionLoads) {
     ...
     */
    shouldScaleUp(metrics, sessionLoads) {
     ...
     */
    shouldScaleDown(metrics, sessionLoads) {
        ...
        const idleSessions = sessionLoads.filter(load => load.activeProjects === 0 && load.utilizationScore < 0.1);
     ...
     */
    shouldRebalance(sessionLoads) {
        if (sessionLoads.length < 2)
            return false;
     ...
     */
    createScaleUpDecision(metrics, _sessionLoads) {
        const sessionsToCreate = Math.min(Math.ceil(metrics.totalProjects / 4) - metrics.activeSessions, this.scalingConfig.maxSessionsPerInstance - metrics.activeSessions);
        return {
            action: 'scale_up',
     ...
     */
    createRebalanceDecision(sessionLoads) {
     ...
     */
    async executeScaleDown(decision) {
    ...
    async executeRebalance(decision) {
        if (!decision.projectsToMove || decision.projectsToMove.length === 0)
            return;
        for (const projectMove of decision.projectsToMove) {
            await this.moveProject(projectMove.projectId, projectMove.fromSessionId, projectMove.toSessionId);
        }
     ...
     */
    async migrateProjectsFromSession(sessionId) {
     ...
     */
    async findBestSessionForProject(_projectId) {
        const sessionLoads = await this.calculateSessionLoads();
        const availableSessions = sessionLoads.filter(load => load.canAcceptMoreProjects);
     ...
     */
    async moveProject(projectId, fromSessionId, toSessionId) {
    ...
    async calculateSessionLoads() {
        ...
        for (const session of activeSessions) {
            ...
            const utilizationScore = Math.min((activeProjects / 4) * 0.5 + (totalCpuUsage / 100) * 0.3 + (totalMemoryUsage / 1000) * 0.2, 1.0);
     ...
     */
    getAllProjectActivities(sessions) {
        return sessions.flatMap(session => session.window.panels.map(panel => panel.projectActivity));
    }
    /**
     ...
     */
    calculateUtilizationVariance(sessionLoads) {
        if (sessionLoads.length < 2)
            return 0;
     ...
     */
    calculateOptimalProjectDistribution(sessionLoads) {
        // Simplified rebalancing - move projects from overloaded to underloaded sessions
        const overloaded = sessionLoads.filter(load => load.utilizationScore > 0.8 || load.activeProjects > this.loadBalancingConfig.maxProjectsPerSession * 0.8);
        const underloaded = sessionLoads.filter(load => load.utilizationScore < 0.4 && load.canAcceptMoreProjects);
        ...
        if (!mostOverloaded || !leastLoaded)
            return [];
        // In a real implementation, would identify specific projects to move
        return [{
                projectId: 'example-project', // Placeholder
     ...
     */
    createSessionParamsForScaling(startProjectNumber) {
     ...
     */
    mergeLoadBalancingConfig(config) {
        const defaultConfig = {
            strategy: 'hybrid',
...
//# sourceMappingURL=SessionScalingManager.js.map