...
// ============================================================================
function generateTestProjectContext(projectType = 'web-app', complexity = 'medium') {
    return {
...
function generateTestPerformanceMetrics(successRate = 0.8, quality = 0.7) {
    return {
        successRate,
...
function generateTestFeedback(agentId, dnaId, taskId, context, outcome = 'success') {
    const baseMetrics = generateTestPerformanceMetrics(outcome === 'success' ? 0.9 : outcome === 'failure' ? 0.3 : 0.6, outcome === 'success' ? 0.8 : outcome === 'failure' ? 0.4 : 0.6);
    return {
        taskId,
        agentId,
        dnaId,
...
`;
    ...
    async testAgentInitialization() {
        ...
        try {
            const agentId = 'agent_test_001';
            const projectContext = generateTestProjectContext('web-app', 'medium');
            const taskId = 'task_001';
            const result = await this.orchestrator.initializeAgent(agentId, projectContext, taskId);
            const success = result.success &&
                result.dnaId !== '' &&
                result.fitnessScore >= 0 &&
                result.reason.length > 0;
            this.recordTest('Agent Initialization', success, success ? 'Agent successfully initialized with DNA' : 'Agent initialization failed', Date.now() - startTime, {
                agentId,
                dnaId: result.dnaId,
            ...
            this.recordTest('Agent Initialization', false, `Test failed with error: ${error instanceof Error ? error.message : 'Unknown error'}`, Date.now() - startTime);
        }
    }
    async testTaskCompletionEvolution() {
        const startTime = Date.now();
        try {
            const agentId = 'agent_test_002';
            ...
            const evolutionResult = await this.orchestrator.processTaskCompletion(agentId, taskId, feedback);
            const success = evolutionResult.fitnessImprovement >= 0 &&
                evolutionResult.knowledgeExtracted >= 0;
            this.recordTest('Task Completion Evolution', success, success ? 'Task completion processed and evolution triggered' : 'Evolution processing failed', Date.now() - startTime, {
            ...
            this.recordTest('Task Completion Evolution', false, `Test failed with error: ${error instanceof Error ? error.message : 'Unknown error'}`, Date.now() - startTime);
        }
    }
    async testCrossProjectLearning() {
        ...
        try {
            ...
            const agent1 = 'agent_cross_001';
            const agent2 = 'agent_cross_002';
            ...
            const feedback1 = generateTestFeedback(agent1, result1.dnaId, 'task_cross_001', project1, 'success');
            await this.orchestrator.processTaskCompletion(agent1, 'task_cross_001', feedback1);
            ...
            const availableKnowledge = await this.orchestrator.findKnowledgeForAgent(agent2, project2.id, project2, mockDna);
            const success = result1.success && result2.success;
            this.recordTest('Cross-Project Learning', success, success ? 'Cross-project learning setup successful' : 'Cross-project learning failed', Date.now() - startTime, {
            ...
            this.recordTest('Cross-Project Learning', false, `Test failed with error: ${error instanceof Error ? error.message : 'Unknown error'}`, Date.now() - startTime);
        }
    }
    async testRealTimeMetrics() {
        ...
        try {
            ...
            this.recordTest('Real-time Metrics', success, success ? 'Real-time metrics functioning properly' : 'Metrics system malfunction', Date.now() - startTime, {
            ...
            this.recordTest('Real-time Metrics', false, `Test failed with error: ${error instanceof Error ? error.message : 'Unknown error'}`, Date.now() - startTime);
        }
    }
    async testWebSocketIntegration() {
        const startTime = Date.now();
        try {
            const connectionId = `ws_test_${Date.now()}`;
            ...
            this.recordTest('WebSocket Integration', success, success ? 'WebSocket integration working correctly' : 'WebSocket integration failed', Date.now() - startTime, {
            ...
            this.recordTest('WebSocket Integration', false, `Test failed with error: ${error instanceof Error ? error.message : 'Unknown error'}`, Date.now() - startTime);
        }
    }
    async testKnowledgeTransfer() {
        ...
        try {
            ...
            this.recordTest('Knowledge Transfer', success, success ? 'Knowledge transfer system operational' : 'Knowledge transfer system failure', Date.now() - startTime, {
            ...
            this.recordTest('Knowledge Transfer', false, `Test failed with error: ${error instanceof Error ? error.message : 'Unknown error'}`, Date.now() - startTime);
        }
    }
    async testPatternMatching() {
        ...
        try {
            ...
            const agent = 'agent_pattern_test';
            ...
            this.recordTest('Pattern Matching', success, success ? 'Pattern matching integrated successfully' : 'Pattern matching integration failed', Date.now() - startTime, {
                agentInitialized: initResult.success,
                dnaId: initResult.dnaId,
            });
        }
        catch (error) {
            this.recordTest('Pattern Matching', false, `Test failed with error: ${error instanceof Error ? error.message : 'Unknown error'}`, Date.now() - startTime);
        }
    }
    async testSystemHealth() {
        ...
        try {
            ...
            this.recordTest('System Health', success, success ? `System health monitoring active (score: ${healthScore.toFixed(3)})` : 'System health monitoring failed', Date.now() - startTime, {
                healthScore,
            ...
            this.recordTest('System Health', false, `Test failed with error: ${error instanceof Error ? error.message : 'Unknown error'}`, Date.now() - startTime);
        }
    }
    recordTest(name, success, message, duration, details) {
...
// MAIN TEST EXECUTION
// ============================================================================
export async function runEvolutionIntegrationTest() {
