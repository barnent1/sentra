...
import { EvolutionIntegrationService } from './evolution-service';
import { PatternMatchingService } from './pattern-matching';
import { EvolutionMetricsService } from './metrics-service';
...
export class EvolutionOrchestrator extends EventEmitter {
    ...
    constructor(config = {}) {
        super();
        this.id = `orchestrator_${Date.now()}_${Math.random().toString(36).slice(2)}`;
        this.config = {
            evolution: {},
            patternMatching: {},
            metrics: {},
     ...
     */
    async initializeAgent(agentId, projectContext, taskId, seedPatterns) {
        try {
            // Initialize DNA through evolution service
            const result = await this.evolutionService.initializeAgentDna(agentId, projectContext, taskId, seedPatterns);
            if (result.success) {
                // Record metrics
     ...
     */
    async processTaskCompletion(agentId, taskId, feedback) {
        try {
            // Process through evolution service
            const evolutionResult = await this.evolutionService.processTaskFeedback(agentId, taskId, feedback);
            // Record performance metrics
            this.metricsService.recordAgentPerformance(agentId, feedback.dnaId, feedback.context.id, taskId, feedback.metrics);
            ...
            if (this.config.knowledgeTransfer && evolutionResult.success) {
                try {
                    ...
                    const extractedKnowledge = await this.knowledgeTransferService.extractKnowledge(agentId, feedback.dnaId, feedback.context.id, taskId, [], // Would pass actual mutations
                    evolutionResult.success ? 0.1 : 0, feedback.context);
     ...
     */
    async findKnowledgeForAgent(agentId, projectId, context, currentDna) {
        if (!this.config.knowledgeTransfer)
            return [];
        return this.knowledgeTransferService.findRelevantKnowledge(agentId, projectId, context, currentDna);
    }
    /**
     ...
     */
    registerWebSocketConnection(connectionId, clientType = 'api') {
        if (this.config.realTimeUpdates) {
    ...
    setupServiceIntegration() {
        ...
        this.evolutionService.on('dna_evolved', (event) => {
            this.metricsService.recordEvolutionEvent(event.agentId, event.evolvedDnaId, event.originalDnaId, event.originalDnaId, // Mock project ID
            event.fitnessImprovement, event.generation, 'fitness_improvement');
        });
