...
class EvolutionaryLearningDemonstration {
    ...
    async demonstrateLearning() {
        ...
        const webAgent = 'agent_web_learner';
        const apiAgent = 'agent_api_learner';
        const cliAgent = 'agent_cli_learner';
        ...
        for (let iteration = 1; iteration <= totalIterations; iteration++) {
            ...
            for (const agent of agents) {
                const taskId = `task_${agent.type}_${iteration}`;
                ...
                const feedback = this.createTaskFeedback(agent.id, agent.dnaId, taskId, agent.project, performance, iteration > 6 ? 'success' : iteration > 3 ? 'partial' : 'success' // Mix of outcomes
                );
    ...
    createProjectContext(type, complexity, techStack) {
        return {
            id: `project_${type}_${Date.now()}`,
            projectType: type,
            techStack,
            complexity,
    ...
    calculateTaskDifficulty(iteration, totalIterations) {
    ...
    generateLearningPerformanceMetrics(iteration, totalIterations, difficulty) {
    ...
    createTaskFeedback(agentId, dnaId, taskId, context, metrics, outcome) {
        return {
            taskId,
            agentId,
            dnaId,
    ...
    async demonstrateCrossProjectLearning(agents) {
        ...
        for (const sourceAgent of agents) {
            for (const targetAgent of agents) {
                ...
                const mockDna = {
                    ...
                    birthContext: { trigger: 'initial_spawn', sourceAgentId: targetAgent.id, creationReason: 'Demo', initialPerformance: {} },
                ...
                const availableKnowledge = await this.orchestrator.findKnowledgeForAgent(targetAgent.id, targetAgent.project.id, targetAgent.project, mockDna);
...
//# sourceMappingURL=learning-demonstration.js.map