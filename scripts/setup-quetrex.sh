...
##############################################################################
# Quetrex AI-Powered SaaS Factory - Complete System Setup
##############################################################################
#
# This script sets up the complete Quetrex development environment including:
...
create_directory_structure() {
    ...
    # Create .quetrex directory structure
    log_info "Creating .quetrex directories..."

    mkdir -p .quetrex/architect-sessions
    mkdir -p .quetrex/scripts
    mkdir -p .quetrex/memory
    mkdir -p .quetrex/metrics
    mkdir -p .quetrex/specs
    mkdir -p .quetrex/telemetry

    log_success "Created .quetrex/ directory structure"
    ...
    if [ -f ".gitignore" ]; then
        # Add Quetrex-specific ignores if not already present
        if ! grep -q "# Quetrex runtime" .gitignore 2>/dev/null; then
            cat >> .gitignore <<'EOF'

# Quetrex runtime
.quetrex/progress.json
.quetrex/architect-sessions/*/session-*.md
.quetrex/metrics/*.json
.quetrex/telemetry/*.log
...
.venv/
EOF
            log_success "Updated .gitignore with Quetrex patterns"
        else
            log_info ".gitignore already contains Quetrex patterns"
        fi
    fi
}
...
# Quetrex AI-Powered SaaS Factory - Python Dependencies
requests>=2.31.0
pyyaml>=6.0.1
python-dotenv>=1.0.0
rich>=13.7.0
...
test_skills_activation() {
    ...
    echo "  3. Skill reads .quetrex/memory/patterns.md and session history"
    echo ""
    ...
    if [ -d ".quetrex/architect-sessions" ]; then
        SESSION_COUNT=$(find .quetrex/architect-sessions -name "session-*.md" 2>/dev/null | wc -l)
...
print_success_message() {
    ...
    echo -e "${GREEN}Quetrex AI-Powered SaaS Factory is ready!${NC}"
    echo ""
    echo -e "${CYAN}Next Steps:${NC}"
    echo ""
    echo -e "${YELLOW}For NEW Projects:${NC}"
    echo "  python .quetrex/scripts/init-project.py --name \"your-project-name\""
    echo ""
    echo -e "${YELLOW}For EXISTING Projects:${NC}"
    echo "  python .quetrex/scripts/init-existing-project.py"
    echo ""
    ...
    echo "  > Skills auto-load patterns from .quetrex/memory/"
    echo ""
    echo -e "${CYAN}Documentation:${NC}"
    echo "  .quetrex/README.md    # Quetrex system overview"
...
main() {
    ...
    echo -e "${PURPLE}╔═══════════════════════════════════════════════════════════╗${NC}"
    echo -e "${PURPLE}║                                                           ║${NC}"
    echo -e "${PURPLE}║      Quetrex AI-Powered SaaS Factory Setup v1.0.0         ║${NC}"
    echo -e "${PURPLE}║                                                           ║${NC}"
    echo -e "${PURPLE}╚═══════════════════════════════════════════════════════════╝${NC}"
    echo ""
    ...
    if [ ! -f "package.json" ]; then
        error_exit "Please run this script from the Quetrex project root directory"
    fi
